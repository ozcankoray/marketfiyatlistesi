import time
from src.core import config
from src.core.database import init_db, insert_products_batch
from src.api.client import fetch_products_from_api
from src.parsers.product_parser import parse_products_data
from src.core.tuik_sepeti import TUIK_GIDA_SEPETI
from src.utils.logger import logger
from src.analytics.statistics import PriceAnalytics


def get_products_from_category(api_category_name: str) -> list:
    """
    Belirtilen tek bir API kategorisi için tüm sayfalardaki ham ürün verilerini çeker.
    
    Args:
        api_category_name: API kategori adı
    
    Returns:
        Ham ürün listesi
    """
    all_products_in_category = []
    current_page = 0

    while True:
        logger.info(f"    -> '{api_category_name}' kategorisi için Sayfa {current_page + 1} taranıyor...")

        payload = config.get_category_payload(category_name=api_category_name, page=current_page)
        raw_data = fetch_products_from_api(config.CATEGORY_SEARCH_URL, config.HEADERS, payload)

        products_list = raw_data.get('content', []) if raw_data else []

        if products_list:
            all_products_in_category.extend(products_list)
            current_page += 1
            # Rate limiting handled by the API client now, no need for sleep here
        else:
            logger.info(f"    -> '{api_category_name}' kategorisinde başka ürün bulunamadı.")
            break

    return all_products_in_category


def main():
    """
    Uygulamanın ana iş akışı.
    TÜİK gıda sepetindeki tüm maddeleri tarayıp veritabanına kaydeder.
    """
    start_time = time.time()
    logger.info("🚀 Program Başlatılıyor...")

    try:
        init_db()
    except Exception as e:
        logger.critical(f"Veritabanı başlatma hatası: {e}")
        return

    logger.info(f"\nAdım 2: TÜİK Gıda Sepeti taranıyor. Toplam {len(TUIK_GIDA_SEPETI)} madde işlenecek.")

    total_products_saved = 0
    failed_categories = []

    for i, madde_item in enumerate(TUIK_GIDA_SEPETI, 1):
        madde_adi = madde_item['madde_adi']
        api_kategori_adi = madde_item.get("api_kategori_adi")

        logger.info(f"\n({i}/{len(TUIK_GIDA_SEPETI)}) İşlenen Madde: '{madde_adi}' (Kategori: '{api_kategori_adi}')")

        if not api_kategori_adi:
            logger.warning(f"  ⚠️  '{madde_adi}' için 'api_kategori_adi' tanımlanmamış. Bu madde atlanıyor.")
            continue

        try:
            ham_urun_listesi = get_products_from_category(api_kategori_adi)

            if not ham_urun_listesi:
                logger.warning(f"  ⚠️  '{api_kategori_adi}' kategorisi için hiç ürün bulunamadı.")
                continue

            filtrelenmis_urunler = parse_products_data(ham_urun_listesi, madde_item)

            if filtrelenmis_urunler:
                insert_products_batch(filtrelenmis_urunler)
                count = len(filtrelenmis_urunler)
                total_products_saved += count
                logger.info(f"  ✔️  '{madde_adi}' için {count} ürün filtrelendi ve veritabanına kaydedildi.")
            else:
                logger.warning(f"  ⚠️  '{madde_adi}' için filtrelenecek uygun ürün bulunamadı.")
                
        except Exception as e:
            logger.error(f"  ❌ '{madde_adi}' işlenirken hata oluştu: {e}")
            failed_categories.append((madde_adi, str(e)))
            continue

    end_time = time.time()
    total_time = end_time - start_time

    logger.info("\n" + "=" * 50)
    logger.info("🎉 TÜM İŞLEMLER TAMAMLANDI 🎉")
    logger.info(f"-> Toplam {total_products_saved} adet ürün verisi veritabanına aktarıldı.")
    logger.info(f"-> Toplam süre: {total_time:.2f} saniye ({total_time/60:.1f} dakika).")
    
    if failed_categories:
        logger.warning(f"-> {len(failed_categories)} kategori işlenemedi:")
        for madde, error in failed_categories[:5]:  # İlk 5 hatayı göster
            logger.warning(f"   - {madde}: {error[:100]}")
    
    logger.info("=" * 50)
    
    # Özet raporu yazdır
    if total_products_saved > 0:
        try:
            PriceAnalytics.print_summary_report()
        except Exception as e:
            logger.error(f"Rapor oluşturulurken hata: {e}")


if __name__ == "__main__":
    main()